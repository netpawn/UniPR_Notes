...
int main() {
 ...
 A* a = new A; //costruzione A
 B b; //costruzione B
 C c; //costruzione C
 A& ra_b = b; //punta a ogg A contenuto in b (statico A, dinamico B)
 B& rb_b = b; //punta a b (st=din)
 A& ra_c = c; //punta a ogg A contenuto in c
 B& rb_c = c; //punta a ogg B in c
 cout << "=== 1 ===" << endl;
 ra_b.f(1); 
//B::f(int)
 rb_b.g(1);
...
//A::f(double)
 static_cast<A*>(&c)->f(1); //st=A din=C
//A::f(int)
 static_cast<B*>(&c)->g(1.2);
//C::g(int)
 cout << "=== 3 ===" << endl;
